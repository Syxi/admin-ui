...
<template>
  <div class="app-container">
    ...
    <!-- 磁盘详情 -->
    <el-card shadow="never" class="mt-5 disk-detail-card">
      <template #header>
        <div class="card-header">
          <span>磁盘详情</span>
        </div>
      </template>
      <el-table :data="systemInfo?.disks" style="width: 100%" class="disk-table">
        <el-table-column prop="name" label="磁盘名称" width="150" />
        <el-table-column prop="mountPoint" label="挂载点" width="200" />
        <el-table-column prop="totalSpace" label="总容量(GB)" width="120">
          <template #default="scope">
            {{ scope.row.totalSpace?.toFixed(2) }}
          </template>
        </el-table-column>
        <el-table-column prop="usedSpace" label="已使用(GB)" width="120">
          <template #default="scope">
            {{ scope.row.usedSpace?.toFixed(2) }}
          </template>
        </el-table-column>
        <el-table-column prop="freeSpace" label="空闲(GB)" width="120">
          <template #default="scope">
            {{ scope.row.freeSpace?.toFixed(2) }}
          </template>
        </el-table-column>
        <el-table-column label="使用率" width="120">
          <template #default="scope">
            <el-progress 
              :percentage="scope.row.totalSpace ? 
                Math.round((scope.row.usedSpace / scope.row.totalSpace) * 100) : 0" 
              :status="Math.round((scope.row.usedSpace / scope.row.totalSpace) * 100) > 80 ? 'exception' : ''"
            />
          </template>
        </el-table-column>
      </el-table>
